...
namespace RonSijm.Blazyload.MicrosoftServiceProvider.ServiceLookup
{
    internal sealed class CallSiteValidator: CallSiteVisitor<CallSiteValidator.CallSiteValidatorState, Type>
    {
                ...
                && _scopedServices.TryGetValue(serviceType, out var scopedService))
            {
                if (serviceType == scopedService)
                {
                    throw new InvalidOperationException(
                        StringResources.Format("DirectScopedResolvedFromRootException", serviceType, nameof(ServiceLifetime.Scoped).ToLowerInvariant()));
                }

                throw new InvalidOperationException(
                    StringResources.Format("ScopedResolvedFromRootException", serviceType, scopedService, nameof(ServiceLifetime.Scoped).ToLowerInvariant()));
            }
        }
        ...
        protected override Type VisitScopeCache(ServiceCallSite scopedCallSite, CallSiteValidatorState state)
        {
            ...
            {
                throw new InvalidOperationException(StringResources.Format("ScopedInSingletonException",
